...
const ROWS = [
  { request: "Create", type: "Consent", title: "Biomarker Research Consent", submittedBy: "Jane Doe", status: "Pending", lastUpdated: "04/28/2026" },
  { request: "Edit", type: "Consent", title: "Germline Research Consent", submittedBy: "Marcus Hill", status: "Pending", lastUpdated: "04/20/2026" },
  { request: "Edit", type: "Consent", title: "Standard Genetic Testing Consent", submittedBy: "Sarah Chen", status: "Approved", lastUpdated: "04/10/2026" },
];
...
const TOAST_MESSAGES: Record<string, { text: string; type: "success" | "warning" }> = {
  ...
  created: { text: "The consent has been created successfully.", type: "success" },
  submitted: { text: "The edit request has been submitted successfully.", type: "success" },
};

function ConsentsAdminStatusQueueInner() {
  ...
  useEffect(() => {
    ...
    if (param && TOAST_MESSAGES[param]) {
      ...
      router.replace("/consents-admin-status-queue", { scroll: false });
    }
  }, [searchParams, router]);
        ...
        <div className="flex flex-col w-full max-w-[1280px] mx-auto" style={{ paddingTop: 32, paddingBottom: 60, paddingLeft: 64, paddingRight: 64, gap: 32 }}>
          ...
          <div className="flex flex-col w-full">
            ...
            {sortedRows.map((row, i) => (
              <div key={i} className="flex items-center w-full bg-bg-page border-b border-border-subtle hover:bg-bg-body" style={{ paddingTop: 16, paddingBottom: 16 }}>
                ...
                <div className="flex items-center min-w-0" style={{ flex: 2, paddingLeft: 8, paddingRight: 8 }}><span style={cellTextStyle}>{row.submittedBy}</span></div>
                ...
                <div className="flex items-center justify-center shrink-0" style={{ width: 140, paddingLeft: 8, paddingRight: 8 }}><span style={{ ...cellTextStyle, textAlign: "center" }}>{row.lastUpdated}</span></div>
                <div className="flex items-center justify-center shrink-0" style={{ width: 140, paddingLeft: 8, paddingRight: 8 }}>
                  <Link
                    href="/consents-admin-review"
...
export default function ConsentsAdminStatusQueue() {
  return (
    <Suspense>
      <ConsentsAdminStatusQueueInner />
    </Suspense>
  );
}

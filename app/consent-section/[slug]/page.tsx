...
interface CardProps {
  card: ContentSectionCard;
  visibleSections: VisibleSections;
  visibleChannels: Set<DistributionChannel>;
  showEdit: boolean;
}

function ConsentCardComponent({ card, visibleSections, visibleChannels, showEdit }: CardProps) {
  const shownChannels = ALL_CHANNELS.filter((ch) => visibleChannels.has(ch));
  const paragraphs = card.portal.split("\n\n");
  ...
  const consentLanguageNode = visibleSections.consentLanguage ? (
    <div className="flex flex-col" style={{ gap: 8 }}>
      ...
      {card.isGap ? (
        ...
        <div className="flex flex-col">{renderLanguage(paragraphs)}</div>
      )}
    </div>
  ) : null;
...
function CustomizeViewPanel({
  allConsentNames, visibleConsents, setVisibleConsents,
  ...
  return (
    <div style={{ width: "100%", background: "white", border: "1px solid var(--borders-border-subtle)", borderRadius: 4, padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div className="flex items-center justify-end w-full" style={{ gap: 16 }}>
        <button onClick={onReset} className="flex items-center justify-center rounded-[4px]" style={{ height: 36, paddingLeft: 16, paddingRight: 16, border: "1px solid var(--text-text-primary)", background: "white" }}>
...
export default function ConsentSectionSlugPage() {
  ...
  const visibleCards = cards.filter((c) => visibleConsents.has(c.consentName));
    ...
    return (
      <div className="flex flex-col min-h-screen bg-bg-page">
        ...
        <main className="flex flex-col w-full max-w-[1280px] mx-auto" style={{ paddingTop: 64, paddingLeft: 64 }}>
          <p style={bodyText}>Consent section not found.</p>
        </main>
      </div>
    );
  }
            ...
            <div className="flex items-center justify-between w-full" style={{ gap: 16 }}>
              ...
              <div className="flex items-center shrink-0" style={{ gap: 24 }}>
                <div className="flex items-center" style={{ gap: 6 }}>
                  {[visibleCards.length, cards.length].map((n, i) => (
                    <span key={i} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 24, height: 24, borderRadius: 9999, background: "var(--tag-tag-default-bg)", fontFamily: "var(--font-barlow), sans-serif", fontWeight: 600, fontSize: 13, lineHeight: "18px", color: "var(--text-text-primary)", padding: "0 6px" }}>{n}</span>
                  )).reduce((acc, el, i) => i === 0 ? [el] : [...acc, <span key={`of-${i}`} style={{ fontFamily: "var(--font-barlow), sans-serif", fontWeight: 400, fontSize: 15, lineHeight: "20px", color: "var(--text-text-primary)" }}>of</span>, el], [] as React.ReactNode[])}

...
function ActionsMenu({ showEdit, onOpenChange }: { showEdit: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <div className="relative flex items-center" style={{ gap: 4 }} ref={ref}>
      ...
      {open && (
        <div className="bg-bg-page border border-border-subtle shadow-dropdown" style={{
          position: "absolute",
          top: "calc(100% + 4px)",
          right: 0,
          borderRadius: 4,
...
function ConsentCardComponent({ card, visibleSections, visibleChannels, showEdit }: CardProps) {
  ...
  return (
    <div className="flex flex-col border border-border-subtle rounded-[4px]" style={{ alignSelf: "start", position: "relative", zIndex: menuOpen ? 10 : 0 }}>
      <div style={{ background: card.status === "Pending" ? "var(--button-caution-btn-caution-bg)" : "var(--deepblue-color-primary-500)", padding: "8px 12px", minHeight: 88, display: "flex", alignItems: "center", borderRadius: "4px 4px 0 0" }}>

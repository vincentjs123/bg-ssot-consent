...
function ActionsMenu({ showEdit }: { showEdit: boolean }) {
  ...
  return (
    <div className="relative flex items-center" style={{ gap: 4 }} ref={ref}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, background: "white", border: "1px solid var(--borders-border-subtle)", borderRadius: 4, boxShadow: "0px 4px 12px var(--bg-ssot-shadow-dropdown)", zIndex: 20, minWidth: 160, padding: "4px 0" }}>
          {items.map((item) => (
            <Link
              key={item}
